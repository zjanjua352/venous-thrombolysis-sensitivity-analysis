% =========================================================================
% Script Name: sobol_stage2_runner.m
% Description: Sobol Variance-Based Sensitivity Analysis - Stage 2 (Runner)
%              Executes the N*(p+2) Saltelli matrix using thread-safe parfor,
%              checkpointing, and batch pre-generation of systemic boundaries.
%
...
if ~exist('sobol_design.mat', 'file')
    error('Blueprint missing: Please execute "sobol_stage1_planner.m" first.');
end
load('sobol_design.mat');
...
if isempty(poolobj)
    parpool('local', 6); % Upgraded to 6 workers for 32GB RAM workstation
end
...
fprintf('--- Phase 2: Executing Parallel Sobol Simulations ---\n');
tic;
...
fprintf('\n');
toc;
fprintf('Sobol execution queue complete. Results stored in "%s".\n', results_dir);
